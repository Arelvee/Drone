...
import csv
import logging
import random  # <-- added for simulated temps
...
# ============================================================
        ...
        try:
            ...
            if self.cap is None:
                messagebox.showerror("Camera Error", 
                                   "Cannot access camera or video. Please check:\n"
                                   "1. Video file exists or camera is connected\n"
            ...
            self.status_var.set("🎥 Camera/Video Active - Real-time inspection running...")
    ...
    def capture_frames(self):
        ...
        while self.running and self.cap and self.cap.isOpened():
            try:
                ...
                if not success:
                    # If video file ended, stop gracefully
                    if self.cap and hasattr(self.cap, 'get') and self.cap.get(cv2.CAP_PROP_FRAME_COUNT) > 0:
                        logging.info("🔚 Video file reached end - stopping inspection")
                        self.running = False
                        break
    ...
    # 🔹 Process Frames with YOLO (updated: imgsz=640 and drawing temps)
    def process_frames(self):
        ...
        while self.running:
            try:
                ...
                # 🔹 Run YOLO inference (imgsz increased to 640)
                start_time = time.time()
                results = self.model(frame, conf=0.1, imgsz=640)
                ...
                if results and len(results[0].boxes) > 0:
                    ...
                    # Use a clean copy to draw our overlays (so we can control text positions)
                    annotated_frame = frame.copy()

                    # If boxes_xyxy and all_detections exists, draw them
                    boxes_xyxy = detection_info.get("boxes_xyxy", [])
                    all_dets = detection_info.get("all_detections", [])
                    ...
                    for coords, det in zip(boxes_xyxy, all_dets):
                        try:
                            ...
                            text_top = f"{label} {conf:.1%}"
                            cv2.putText(annotated_frame, text_top, (x1, max(10, y1 - 10)),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
                            # Temperature below the box
                            temp_text = f"{temp:.1f}°C"
                            cv2.putText(annotated_frame, temp_text, (x1, min(annotated_frame.shape[0]-5, y2 + 20)),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2, cv2.LINE_AA)
                        except Exception as draw_e:
                            logging.error(f"Drawing overlay error: {draw_e}")
    ...
    def extract_detection_info(self, results):
        """Extract detection information from YOLO results - PROCESS ALL DETECTIONS
           Also simulate realistic, class-dependent temperatures and return box coords."""
        detection_info = {
            "label": "No Detection",
            "confidence": "0%",
            "temperature": "0.0 °C",
        ...
        try:
            if len(results[0].boxes) > 0:
                ...
                # Temperature ranges per class (realistic, adjustable)
                # You said: not generic 25-70 — so we define per class ranges
                temp_ranges = {
                    0: (35.0, 65.0),  # 1-1A-Fired Wedge Joint-BARE (hotter)
                    1: (30.0, 55.0),  # 1-1B-Fired Wedge Joint-COVERED (slightly cooler)
                    2: (32.0, 60.0),  # 2-2A-Hummer Driven Wedge Joint-BARE
                    3: (28.0, 50.0)   # 2-2B-Hummer Driven Wedge Joint-COVERED
                }
                
                # iterate through boxes; boxes.cls, boxes.conf, boxes.xyxy should be indexable
                for i in range(len(boxes)):
                    # Get confidence and class id
                    try:
                        confidence = float(boxes.conf[i]) if hasattr(boxes, 'conf') else float(boxes[i].conf)
                    except Exception:
                        # fallback: attempt indexing from results
                        confidence = 0.0
                    try:
                        class_id = int(boxes.cls[i]) if hasattr(boxes, 'cls') else int(boxes[i].cls)
                    except Exception:
                        class_id = -1
                    ...
                    # Simulate temperature per class using temp_ranges
                    tr = temp_ranges.get(class_id, (30.0, 60.0))
                    # Increase likelihood of higher temp when confidence is higher
                    low, high = tr
                    # Weighted random influenced by confidence
                    # Map confidence (0..1) to bias in range
                    bias = confidence
                    simulated_temp = random.uniform(low + (high - low) * bias * 0.1, high - (high - low) * (1 - bias) * 0.05)
                    # Small jitter
                    simulated_temp = round(simulated_temp + random.uniform(-0.5, 0.5), 1)
                    
                    # Get bbox coords (xyxy)
                    try:
                        # boxes.xyxy should be indexable (each xs is (x1, y1, x2, y2))
                        raw_coords = boxes.xyxy[i]
                        # Convert to ints robustly
                        coords = tuple(map(int, (raw_coords[0], raw_coords[1], raw_coords[2], raw_coords[3])))
                    except Exception:
                        # fallback default bbox
                        coords = (0, 0, 0, 0)
                    
                    labels.append(class_name)
                    confidences.append(confidence)
                    class_ids.append(class_id)
                    temperatures.append(simulated_temp)
            ...
            try:
                ...
                if not self.result_queue.empty():
                    try:
                        ...
                        if detection_info.get("label", "No Detection") != "No Detection":
